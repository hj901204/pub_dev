...
const actions = {
    setPageLoading: (bool) => (dispatch, getState) => {
        let state = getState()[REDU.PURCHASE_EIDT_TOP_REDU].set("pageLoading", bool);
        dispatch({ type: REDU.PURCHASE_EIDT_TOP_REDU, state });
    },
    setReceiveBtnLoading: (bool) => (dispatch, getState) => {
        let state = getState()[REDU.PURCHASE_EIDT_TOP_REDU].set("receiveBtnLoading", bool);
        dispatch({ type: REDU.PURCHASE_EIDT_TOP_REDU, state });
    },
    receiptHandle: (val) => (dispatch, getState) => {
        let documentNo = getState()[REDU.PURCHASELISTREDU].get('takeOrderPm'),
        ...
        ReqApi.get({
            ...
            if (json.status === 2000) {
                ...
                dispatch(TabsAct.TabRemove('inventoryPurchaseEidt', 'inventoryPurchaseListCont'));
                message.success('收货成功');
                if (getStateFun(getState).get('tabs').some((tab) => {
                    return tab.key == 'inventoryPurchaseListCont'
                })) {
                    dispatch(PurchaseListAct.PurchaseList({ page: 1, pageSize: 10, sourceOrderType: 11 }));
                }
    ...
    preReceiptInfoStatus: () => (dispatch, getState) => { // 查看预收货信息单状态  是否有‘未收货’
        let dataList = getState()[REDU.PURCHASE_EIDT_TABLE_TAB_REDU].getIn(['receiveGoodsTableData', 'dataSource']);
    ...
    storePurchaseWareHouseFromInfo: (formInfo) => (dispatch, getState) => { // store 采购入库单 信息总览
        let state = getState()[REDU.PURCHASE_EIDT_TOP_REDU].setIn(['formInfo'], formInfo);
        dispatch({ type: REDU.PURCHASE_EIDT_TOP_REDU, state });
    },
    ...
    storeOrderInfoTableList: (val) => (dispatch, getState) => { //store 采购入库单 订单信息
        ...
        let state = getState()[REDU.PURCHASE_EIDT_TABLE_TAB_REDU].setIn(['orderInfoTableData', 'dataSource'], list).setIn(['orderInfoTableData', 'paging'], { page, pageSize, total });
        dispatch({ type: REDU.PURCHASE_EIDT_TABLE_TAB_REDU, state })
    },
    ...
    storePreReceiveTableList: (val) => (dispatch, getState) => {// store 采购入库单 预收货信息
        let { list, page, pageSize, total } = val;
        let state = getState()[REDU.PURCHASE_EIDT_TABLE_TAB_REDU].setIn(['receiveGoodsTableData', 'dataSource'], list).setIn(['receiveGoodsTableData', 'paging'], { page, pageSize, total });
        dispatch({ type: REDU.PURCHASE_EIDT_TABLE_TAB_REDU, state })
    },
    tableLoadingForReceiveGoodsTable: (bool) => (dispatch, getState) => {
        let state = getState()[REDU.PURCHASE_EIDT_TABLE_TAB_REDU].setIn(['receiveGoodsTableData', 'tableLoading'], bool)
        dispatch({ type: REDU.PURCHASE_EIDT_TABLE_TAB_REDU, state })
    },
    tablePagingForReceiveGoodsTable: (page) => (dispatch, getState) => {
        let loading = getState()[REDU.PURCHASE_EIDT_TOP_REDU].get("pageLoading"),
            documentNo = getState()[REDU.PURCHASELISTREDU].get('takeOrderPm'),  // 单据号
    ...
    tableLoadingForOrderInfoTable: (bool) => (dispatch, getState) => {
        let state = getState()[REDU.PURCHASE_EIDT_TABLE_TAB_REDU].setIn(['orderInfoTableData', 'tableLoading'], bool)
        dispatch({ type: REDU.PURCHASE_EIDT_TABLE_TAB_REDU, state })
    },
    tablePagingForOrderInfoTable: (page) => (dispatch, getState) => {
        ...
        if (!loading) {
            ...
            // page 参数 
            let state = getState()[REDU.PURCHASE_EIDT_TOP_REDU].set("orderinfoPagePm", pm);
            dispatch({ type: REDU.PURCHASE_EIDT_TOP_REDU, state });
            //
